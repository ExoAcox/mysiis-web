...
const SurveyModal: React.FC<{ user: User }> = ({ user }) => {
    ...
    const process = useModal<Respondent>("dashboard-survey-process");
    ...
    const statusPermit = (sourcename: string) => {
        let questionId: number;
        if(sourcename === "telkomakses"){
            questionId = 182;
        } else if(sourcename === "enciety") {
            questionId = 197;
        }

        const status = respondentWithResponse?.responses.lists
            ?.flatMap(item => item.groups)
            .flatMap(group => group.responses)
            .filter(response => response.questionid === questionId);

        return status && status.length > 0 ? status[0].value : undefined;
    }

    const handleValidate = () => {
        // validate.setData(data);
        const status = statusPermit(data.sourcename!);
        if (data?.sourcename === "telkomakses") {
            if(status === "Diizinkan"){
                validate.setData(data);
            } else if(status === "Tidak diizinkan"){
                reject.setData(data);
            } else {
                process.setData(data);
            }
        } else if(data.sourcename === "enciety"){
            if(status === "Diizinkan"){
                validate.setData(data);
            } else if(status === "Tidak diizinkan"){
                reject.setData(data);
                        ...
                        <Switch>
                            ...
                            >
                                <div className="flex justify-end gap-4">
                                    <When
                                        condition={
                                            !!intersection(["supervisor-survey-mitra"], user.role_keys).length &&
                                            // ["valid-mitra"].includes(data.status!) &&
                                            !["done", "approved", "process", "rejected"].includes(data.mysista_data?.status_permits!)
                                        }
                                    >
                                        <Button onClick={handleValidate}>
                                            {"Validasi " + statusPermit(data.sourcename!)}
                                        </Button>
                                        {/* <Button onClick={handleValidate}>
                                            Validasi
                                        </Button> */}
                                    ...
                                    <When
                                        condition={
                                            !!intersection(["supervisor-survey-mitra"], user.role_keys).length &&
                                            // ["valid"].includes(data.status!) && 
                                            ["process"].includes(data.mysista_data?.status_permits!)
                                        }
                                    >
                                        <Button variant="ghost" onClick={()=> reject.setData(data)}>
                                            {"Ubah Perizinan untuk ditolak"}
                                        </Button>
                                    </When>
                                    <When
                                        condition={
                                            ...
                                            ["process","rejected"].includes(data.mysista_data?.status_permits!)
                                        }
                                    >
                                        <Button onClick={()=> validate.setData(data)}>
                                            {"Ubah Perizinan untuk disetujui"}
                                        </Button>
                                    </When>
                                    <When
                                        condition={
                                            !!intersection(["supervisor-survey-mitra"], user.role_keys).length &&
                                            // ["valid","invalid"].includes(data.status!) && 
                                            ["approved"].includes(data.mysista_data?.status_permits!)
                                        }
                                    >
                                        <Button onClick={()=> process.setData(data)}>
                                            {"Ubah Perizinan menjadi menunggu proses"}
                                        </Button>
                                    </When>
                                </div>
                            </Case>
