...
const Banner = () => {
    ...
    const { data, isLoading, isSuccess } = useBanner({ page: 1, row: 10, status: true });
                        ...
                        {data?.map((banner) => {
                            return (
                                <SwiperSlide key={banner.id}>
                                    <Link href={banner.url ? `/news/${getNewsId(banner.url)}` : "#"}>
                                        <Image
                                            src={banner.image_url}
                                            alt="banner"
                                            fill
                                            className="object-contain rounded-xl"
                                            parentClassName="relative w-full h-[20.625rem] lg:h-[16rem] md:h-[12rem] sm:h-[11rem]"
                                        />
                                    </Link>
                                </SwiperSlide>
                            );
