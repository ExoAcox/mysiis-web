...
import { Polygon, addAssignmentPermit } from "@api/survey-demand/mysiista";
...
const SurveyorAssignmentModal: React.FC<{ user: User; refresh: () => void }> = ({ user, refresh }) => {
    ...
    const handleSubmit = () => {
        ...
        submit.fetch(addAssignmentPermit({ type: form.type, surveyid: 1, supervisorid: user.userId, user_data: user_data, mysista_data }), {
            onResolve: () => {
                closeModal();
                refresh();
            },
