...
import { unBlockUser, User } from "@api/account/user";

const UnblockModal: React.FC<{ refresh: () => void }> = ({ refresh }) => {
    const { data, setModal } = useModal<User>("supervisor-unblock");
    ...
    return (
        <>
            <WarningModal id="supervisor-unblock" variant="accept" message="Yakin ingin terima" name={data.fullname} loading={loading}>
                ...
                <Button
                    ...
                    loading={loading}
                    onClick={() => {
                        submit.fetch(unBlockUser(data.userId), {
                            onResolve: () => {
                                closeModal();
                                refresh();
                            },
