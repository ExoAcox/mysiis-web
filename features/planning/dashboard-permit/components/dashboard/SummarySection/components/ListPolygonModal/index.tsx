...
import { exportSummaryPolygonPermit, getSummaryPolygonPermit, SummaryPemitsData } from "@api/survey-demand/summary";
...
const PolygonDetailModal: React.FC<{ user: User }> = ({ user }) => {
    const { modal, data, setModal } = useModal<SummaryPemitsData>("summary-progress-polygon-permits");
    ...
    const getParams = () => {
        ...
        if(data.type === "regional") {
            params.treg = data.regional;
        } else if(data.type === "witel") {
            params.treg = data.regional;
            params.witel = data.witel;
        }
    ...
    }, [modal, status, statusPermit, reset, filter.page, filter.row]);
    ...
    const handleExport = async () => {
        ...
        const filename = `Summary_progress_permit_${level}_${dayjs(new Date()).format("DDMMYYYY")}.xlsx`;

        try {
            const result = await exportSummaryPolygonPermit({ ...params }); 
            if(result){
            ...
            <div className="p-4">
                ...
                <Table
                    ...
                    columns={[
                        ...
                        { header: "Status Polygon", value: (data) => {
                            return (
                                <div title="Lihat log status" className="flex gap-0">
                                    <Button variant="nude" onClick={()=> modalLogs.setData(data.log_status)}>
                                        <Badge>
                                            {data.polygon?.status}
                                        </Badge>
                                    </Button>
                                    {!data.polygon?.ihld_sent && <FiInfo className="text-red-500" size={15} title="Belum terkirim ke IHLD, masih menunggu perizinan" />}
                                </div>
                            )
                        }},
                        { header: "Status Permit", value: (data) => <BadgeStatusPermit status={data.polygon?.status_permits!} /> },
                        {header: "Aksi", value: (data) => {
                            if(data.polygon?.surveyor === "telkomakses"){
                                return (
                                    <When condition={intersection(user.role_keys, accessNextDesign).length && user.vendor === "Telkom Akses"}>
                                        <Button
                                            className="py-1.5 mx-auto m-0"
                                            disabled={data.polygon?.status !== "done" || data.polygon.status_permits !== "approved"}
                                            onClick={() => setModalConfirm({ status: data.polygon?.status, objectid: data.polygon?.objectid, polygon: data.polygon?.name }) }
                                        >
                                            <span title={data.polygon?.status !== "done" ? "Maaf polygon ini belum ber status done atau belum ada BAKP" : ""}>
                                                Lanjut Design
                                            </span>
                                        </Button>
                                    </When>
...
const BadgeStatusPermit: React.FC<{ status: string }> = ({ status }) => {
    let variant: "default" | "success" | "warning" | "error" | "information";
    if (status === "approved") {
        variant = "success";
